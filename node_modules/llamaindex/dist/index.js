...
__export(src_exports, {
  ...
  AudioSubtitlesReader: () => AudioSubtitlesReader,
  AudioTranscriptParagraphsReader: () => AudioTranscriptParagraphsReader,
  AudioTranscriptReader: () => AudioTranscriptReader,
  AudioTranscriptSentencesReader: () => AudioTranscriptSentencesReader,
  ...
  SimpleMongoReader: () => SimpleMongoReader,
...
var InMemoryFileSystem = class {
  ...
  writeFile(path5, content, options) {
    return __async(this, null, function* () {
      this.files[path5] = import_lodash.default.cloneDeep(content);
    });
  }
  readFile(path5, options) {
    return __async(this, null, function* () {
      ...
      return import_lodash.default.cloneDeep(this.files[path5]);
    });
  }
  access(path5) {
    return __async(this, null, function* () {
  ...
  mkdir(path5, options) {
    return __async(this, null, function* () {
      this.files[path5] = import_lodash.default.get(this.files, path5, null);
    });
  }
};
function getNodeFS() {
...
var TextNode = class _TextNode extends BaseNode {
   ...
   */
  generateHash() {
    const hashFunction = import_crypto_js.default.algo.SHA256.create();
    hashFunction.update(`type=${this.getType()}`);
    hashFunction.update(
      `startCharIdx=${this.startCharIdx} endCharIdx=${this.endCharIdx}`
    );
    hashFunction.update(this.getContent("ALL" /* ALL */));
    return hashFunction.finalize().toString(import_crypto_js.default.enc.Base64);
  }
  getType() {
    return "TEXT" /* TEXT */;
  }
...
var SimpleDocumentStore = class _SimpleDocumentStore extends KVDocumentStore {
  ...
  persist() {
    return __async(this, arguments, function* (persistPath = path2.join(
      ...
      fs2 = fs2 || DEFAULT_FS;
      if (import_lodash2.default.isObject(this.kvStore) && this.kvStore instanceof BaseInMemoryKVStore) {
  ...
  toDict() {
    if (import_lodash2.default.isObject(this.kvStore) && this.kvStore instanceof SimpleKVStore) {
...
var IndexStruct = class {
  constructor(indexId = (0, import_uuid2.v4)(), summary = void 0) {
...
var KVIndexStore = class extends BaseIndexStore {
  ...
  getIndexStruct(structId) {
    return __async(this, null, function* () {
      if (import_lodash3.default.isNil(structId)) {
        ...
        if (import_lodash3.default.isNil(json)) {
          return;
        }
  ...
  getIndexStructs() {
    return __async(this, null, function* () {
      ...
      return import_lodash3.default.values(jsons).map((json) => jsonToIndexStruct(json));
    });
  }
};
...
var SimpleVectorStore = class _SimpleVectorStore {
  ...
  query(query) {
    return __async(this, null, function* () {
      if (!import_lodash4.default.isNil(query.filters)) {
        throw new Error(
...
var MongoDBAtlasVectorSearch = class {
  constructor(init) {
    ...
    if (init.mongodbClient) {
      ...
      this.mongodbClient = new import_mongodb.MongoClient(mongoUri);
    }
...
var import_pg2 = __toESM(require("pgvector/pg"));
...
var PGVectorStore = class {
  ...
  getDb() {
    return __async(this, null, function* () {
      if (!this.db) {
        try {
          const db = new import_pg.default.Client();
          yield db.connect();
          db.query("CREATE EXTENSION IF NOT EXISTS vector");
          yield import_pg2.default.registerType(db);
...
function readImage(input) {
  return __async(this, null, function* () {
    ...
    } else if (import_lodash5.default.isString(input) || input instanceof URL) {
...
function getOpenAISession(options = {}) {
  ...
  let session = (_a = defaultOpenAISession.find((session2) => {
    return import_lodash6.default.isEqual(session2.options, options);
...
function getAnthropicSession(options = {}) {
  ...
  let session = (_a = defaultAnthropicSession.find((session2) => {
    return import_lodash7.default.isEqual(session2.options, options);
  })) == null ? void 0 : _a.session;
  if (!session) {
...
function getPortkeySession(options = {}) {
  ...
  let session = (_a = defaultPortkeySession.find((session2) => {
    return import_lodash8.default.isEqual(session2.options, options);
  })) == null ? void 0 : _a.session;
  if (!session) {
...
var defaultParseChoiceSelectAnswerFn = (answer, numChoices, raiseErr = false) => {
  const lineTokens = answer.split("\n").map((line) => {
    ...
    return lineTokens2;
  }).filter((lineTokens2) => !import_lodash9.default.isNil(lineTokens2));
    ...
    return __async(this, null, function* () {
      ...
      for (let idx = 0; idx < nodeIds.length; idx += this.choiceBatchSize) {
        ...
        const nodeWithScores = choiceNodes.map((node, i) => ({
          node,
          score: import_lodash10.default.get(parseResult, `${i + 1}`, 1)
        }));
  ...
  getRefDocInfo() {
    return __async(this, null, function* () {
      ...
      for (const node of nodes) {
        ...
        if (import_lodash11.default.isNil(refNode)) {
          continue;
        }
        const refDocInfo = yield this.docStore.getRefDocInfo(refNode.nodeId);
        if (import_lodash11.default.isNil(refDocInfo)) {
          continue;
        }
...
var VectorIndexRetriever = class {
  ...
  textRetrieve(query, preFilters) {
    return __async(this, null, function* () {
      const q = yield this.buildVectorStoreQuery(this.index.embedModel, query);
  ...
  buildVectorStoreQuery(embedModel, query) {
    return __async(this, null, function* () {
...
var AssemblyAIReader = class {
  ...
  constructor(assemblyAIOptions) {
    ...
    this.client = new import_assemblyai.AssemblyAI(options);
  }
    ...
    return __async(this, arguments, function* ({
      ...
      try {
        for (var iter = __forAwait(walk(fs2, directoryPath)), more, temp, error; more = !(temp = yield iter.next()).done; more = false) {
          ...
          try {
            const fileExt = import_lodash12.default.last(filePath.split(".")) || "";
            ...
            } else if (!import_lodash12.default.isNil(defaultReader)) {
